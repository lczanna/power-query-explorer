...
function renderDataPanel(){
    ...
    let chips='';
    for(const ws of appState.worksheets){
        ...
        if(appState.worksheets.length>1)chips+=' <span style="opacity:0.5;font-size:10px">'+escapeHtml(ws.fileName)+'</span>';
        chips+='</button>';
    }
    ...
    if(!appState.activeSheet&&appState.worksheets.length>0){
        const first=appState.worksheets[0];
